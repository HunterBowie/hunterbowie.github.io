...
import { Board, getPiece, makePos } from "./core.js";
import { BLACK, KING, PAWN, PieceColor, WHITE } from "./piece.js";
 ...
 */
function getNumKings(color: PieceColor, board: Board): number {
  let number = 0;
 ...
 * Throws BoardStateError if a board with an invalid mailbox is passed.
 */
function areWhitePawnOnImpossibleRank(board: Board): boolean {
  const rankNum = 1;
  for (let fileNum = 1; fileNum <= 8; fileNum++) {
 ...
 * Throws BoardStateError if a board with an invalid mailbox is passed.
 */
function areBlackPawnOnImpossibleRank(board: Board): boolean {
  const rankNum = 8;
  for (let fileNum = 1; fileNum <= 8; fileNum++) {
    const piece = getPiece(makePos(rankNum, fileNum), board);
