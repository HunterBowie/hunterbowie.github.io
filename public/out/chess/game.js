...
import { changeToMove, createBoard, getPiece, isInvalidPos, setPiece, } from "./board.js";
...
export class Game {
    ...
    dropPiece(pos) {
        ...
        const hasPossibleMove = this.possibleMoves.filter((move) => move.end.row === pos.row && move.end.col === pos.col);
        if (isInvalidPos(pos) || hasPossibleMove.length == 0) {
            // return piece to orginal square
            setPiece(this.hand.homePos, this.hand.piece, this.board);
            this.hand = { piece: 0, homePos: null, hoverPoint: null };
            return;
        }
