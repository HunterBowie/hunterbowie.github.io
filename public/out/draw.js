import { CANVAS_MARGIN, CHESS_BOARD_ID, DARK_SQUARE, DRAW_DELAY, HIGHLIGHT_SQUARE, LIGHT_SQUARE, SPECIAL_HIGHLIGHT_SQUARE, } from "./constants.js";
...
function drawBoardTiles() {
    ...
    for (let row = 0; row <= 8; row++) {
        for (let col = 0; col <= 8; col++) {
            ...
            drawRect(color, col * squareWidth, row * squareWidth, squareWidth, squareWidth);
        }
    }
}
/**
...
function drawPossibleMoves(moves) {
    ...
    moves.forEach((move, _) => {
        drawRect(HIGHLIGHT_SQUARE, move.end.col * squareWidth, move.end.row * squareWidth, squareWidth, squareWidth);
    });
}
/**
 ...
 * if it is picked up.
 */
function drawCurrentPieceTile(hand) {
    if (hand.piece == 0) {
        return;
    }
    drawRect(SPECIAL_HIGHLIGHT_SQUARE, hand.homePos.col * getSquareWidth(), hand.homePos.row * getSquareWidth(), getSquareWidth(), getSquareWidth());
}
/**
...
function drawHand(hand) {
    ...
    drawPieceImage(hand.piece, hand.hoverPoint.x - halfPieceWidth, hand.hoverPoint.y - halfPieceWidth);
}
/**
