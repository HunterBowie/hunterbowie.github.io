...
import { getFileNumber, getPiece, getRankNumber, makePos, nextTurn, shiftPos, } from "../core.js";
 ...
 */
export function canCastleKingside(board) {
    const pos = findKingPos(board);
    const endPos = shiftPos(pos, 0, 2);
    const spots = getSpotsFromFileRange(getRankNumber(pos), getFileNumber(pos), getFileNumber(endPos));
 ...
 */
function getSpotsFromFileRange(rankNum, fileStartNum, fileEndNum) {
    let spots = [];
 ...
 */
function containsPieces(spots, board) {
 ...
 * Throws BoardStateError if a board with an invalid mailbox is passed.
 */
function isUnderAttack(spots, board) {
